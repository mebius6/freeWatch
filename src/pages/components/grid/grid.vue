<template>
	<view class="free-watch-grid-container">
		<uni-grid :column="gridColumn" :showBorder="gridShowBorder" :borderColor="gridBorderColor" :hor="gridHor" :ver="gridVer" :square="gridSquare" :highlight="gridHighlight" class="free-watch-grid">
			<uni-grid-item v-for="(item, index) in gridData" :key="index" :img-width="item.imgWidth" :src="item.imgSrc" :hor="item.hor" :ver="item.ver" :marker="item.marker || 'image'" class="free-watch-grid-item" @click.native="gridItem(item)">
