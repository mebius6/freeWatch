<template>
	<view class="free-watch-card">
		...
		<view class="card-right">
			<view v-for="(item, index) in columns" :key="index" class="free-watch-card-right">
				<view class="card-right-item" v-if="!item.render">
					<view v-if="(item.list || []).length">
						<text v-for="(v, i) in item.list || []" :key="i">{{ v }}<text v-if="v" class="dot"></text></text>
					</view>
				</view>
				<view class="card-right-item" v-else>
					<Expand :row="item.data || {}" :index="index" :render="item.render"></Expand>
				</view>
			</view>
		</view>
	</view>
